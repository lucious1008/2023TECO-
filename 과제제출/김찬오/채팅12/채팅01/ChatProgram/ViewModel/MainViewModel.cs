...
namespace ChatProgram.ViewModel
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        private void CreateServer()
        {
            ...
            {
                MessageBox.Show("방에대한 프로퍼티를 모두 입력해주세요.", "채팅방 프로퍼티 오류", MessageBoxButtons.OK);
                return;
            }
            ...
            {
                MessageBox.Show("입장 가능한 인원은 숫자만 입력할 수 있습니다.", "서버 프로퍼티 오류", MessageBoxButtons.OK);
                return;
            }

            if (serverCapacityInteger < 2)
            {
                MessageBox.Show("방 개설 가능  최소 인원은  2명 이상이여야 합니다.", "서버 프로퍼티 오류", MessageBoxButtons.OK);
                return;
            }

            if (serverCapacityInteger > 100)
            {
                MessageBox.Show("방 입장  허용 인원 은 100명 까지 입니다.", "서버 프로퍼티 오류", MessageBoxButtons.OK);
                return;
            }
        ...
        public void ChangeServerStatus_LostConnect()
        {
            chatProgramIsStart = false;
            ChatSettingPanelVisibility = false;
            ConnectStatusText = "방 생성/ 생성 되지않음";
            ConnectStatusColor = Brushes.Red;
            Nickname = "user";
            NicknameColor = Brushes.Green;
            ChatColor = Brushes.Black;
